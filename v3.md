...
> **Changelog from v3 base:** Added `src/search/rerank/` with `Reranker.ts`, `NoOpReranker.ts`, and `CrossEncoderReranker.ts`. Updated folder structure, dependency map, Phase 7 (HybridRetriever wiring and reranker integration), and Phase 8 (QueryEngine flow). A `NoOpReranker` ships in v3; `CrossEncoderReranker` is the extension point for BGE-Reranker / Jina.
...
LLM answer generation is intentionally out of scope. A `PlaceholderLlmProvider` is wired in at Phase 8; swap it with a real provider later. Everything else — extraction, graph, storage, indexing, retrieval, reranking — is fully implemented.
...
│   │   └── HybridRetriever.ts  # BM25 + vector + RRF + reranking    ← updated
│   └── rerank/                                                        ← NEW
│       ├── Reranker.ts             # interface
│       ├── NoOpReranker.ts         # v3 default — identity pass-through
│       └── CrossEncoderReranker.ts # extension point for BGE / Jina
├── expansion/
...
Vector Searcher
  ...
  RRF Fusion
    │
    ▼
  Reranker          ← new stage (NoOpReranker in v3; swap for CrossEncoderReranker later)
    │
    ▼
Graph Expansion
    │
...
export interface SearchHit {
  ...
  score: number;
  chunk: ChunkRecord;
  source: 'bm25' | 'vector' | 'hybrid' | 'reranked';  // ← 'reranked' added
}
...
export const ConfigSchema = z.object({
  ...
  // Reranking                                                          ← NEW
  rerankTopK: z.number().default(10),
});
...
- Approximate token count: `Math.ceil(words.length * 1.3)` where `words = sourceText.split(/\s+/)`.
- If it fits within `config.chunkMaxTokens`, return a single chunk.
- Otherwise, split into overlapping windows:
  ...
  - For each window: assign `chunkIndex`, compute `startLine` / `endLine` by counting newlines in the text before the window offset, set `contentHash = sha1(windowText)`.
- Set `totalChunks` on all chunks from this node after the full split is known.
- Leave `embeddingText: ''` — filled in by `EmbeddingTextBuilder` in Phase 5.
...
## Phase 7 — Hybrid Retrieval, Reranking & Graph Expansion

**Goal:** Fuse BM25 and vector results via RRF, then pass through a pluggable reranker. Expand the graph around seed nodes with a configurable budget. Assemble the final context string.
...
- For each hit at rank `r` (1-based) in each list: `rrf_score += 1 / (k + r)`.
- Sum across all lists per unique `chunkId`.
- Sort descending. Return top `topK` with `source: 'hybrid'`.

---

### `src/search/rerank/Reranker.ts` *(NEW)*

The shared interface for all rerankers. Accepting `hits` already hydrated with `ChunkRecord` means implementors never need to touch the database.

```typescript
import type { SearchHit } from '../../types/index.js';

/**
 * Reranker — takes RRF-fused hits and re-scores them for a given query.
 *
 * Contract:
 *  - Input hits are already hydrated (chunk.text is populated).
 *  - Output length ≤ topK.
 *  - Output hits have source: 'reranked'.
 *  - Implementations MUST be stateless with respect to the index;
 *    all state lives in the constructor (model, endpoint, config).
 */
...
### `src/search/rerank/NoOpReranker.ts` *(NEW)*

Default implementation shipped in v3. Passes hits through unchanged (preserving the RRF ordering) and stamps `source: 'reranked'`. Zero latency, zero external calls — safe to use during development and integration testing.
...
/**
 * NoOpReranker — identity pass-through.
 *
 * Returns the top-K hits from the RRF list in their existing order,
 * with source updated to 'reranked'. Use this in v3 until a real
...
export class NoOpReranker implements Reranker {
  async rerank(
    ...
    return hits
      .slice(0, topK)
      .map(h => ({ ...h, source: 'reranked' as const }));
  }
}
```
...
### `src/search/rerank/CrossEncoderReranker.ts` *(NEW)*

Extension point for neural cross-encoder rerankers (BGE-Reranker-v2-M3, Jina Reranker v2, Cohere Rerank, etc.). The skeleton below shows the interface contract and the score-normalisation pattern; fill in the provider-specific HTTP call for each backend.
...
export interface CrossEncoderConfig {
   ...
   *   'bge'   — local BGE-Reranker served via a REST endpoint
   *   'jina'  — Jina Reranker v2 API (api.jina.ai)
   *   'cohere'— Cohere Rerank API
   */
  provider: 'bge' | 'jina' | 'cohere';

  /** Base URL for 'bge' (e.g. http://localhost:8080) or full endpoint override. */
  endpoint?: string;

  /** API key — read from env if omitted (JINA_API_KEY / COHERE_API_KEY). */
  apiKey?: string;
...
/**
 * CrossEncoderReranker — neural reranking via a cross-encoder model.
 *
 * Swap in for NoOpReranker in buildQueryEngine() when a backend is ready:
 *
 *   const reranker = new CrossEncoderReranker({ provider: 'jina' });
 *
 * The reranker calls the configured backend with (query, passage) pairs,
 * normalises scores to [0, 1] via min-max, and returns the top-K hits
 * stamped with source: 'reranked'.
 */
export class CrossEncoderReranker implements Reranker {
  ...
  async rerank(
    ...
    // ── 1. Build (query, passage) pairs ──────────────────────────────────
    const passages = hits.map(h => h.chunk.text);

    // ── 2. Call the backend ───────────────────────────────────────────────
    const rawScores = await this.callBackend(query, passages);
    ...
    const scored = hits.map((hit, i) => ({
      ...hit,
      score: (rawScores[i] - min) / range,
      source: 'reranked' as const,
    }));

    // ── 4. Sort descending and return topK ───────────────────────────────
    return scored
  ...
  private async callBackend(query: string, passages: string[]): Promise<number[]> {
    switch (this.config.provider) {
      case 'bge':    return this.callBge(query, passages);
      case 'jina':   return this.callJina(query, passages);
      case 'cohere': return this.callCohere(query, passages);
    }
  }

  /**
   * BGE-Reranker — expects a local REST server (e.g. FlagEmbedding's API server).
   * POST /rerank  { query: string, passages: string[] }
   * Response: { scores: number[] }
   */
  private async callBge(query: string, passages: string[]): Promise<number[]> {
    ...
    if (!res.ok) throw new Error(`BGE reranker error: ${res.status} ${res.statusText}`);
    const data = await res.json() as { scores: number[] };
    return data.scores;
  }

  /**
   * Jina Reranker v2 — https://api.jina.ai/v1/rerank
   * Env: JINA_API_KEY
   */
  private async callJina(query: string, passages: string[]): Promise<number[]> {
    ...
    if (!res.ok) throw new Error(`Jina reranker error: ${res.status} ${res.statusText}`);
    const data = await res.json() as { results: { index: number; relevance_score: number }[] };

    // Jina returns results in descending score order with original index.
   ...
   * Env: COHERE_API_KEY
   */
  private async callCohere(query: string, passages: string[]): Promise<number[]> {
    ...
    if (!res.ok) throw new Error(`Cohere reranker error: ${res.status} ${res.statusText}`);
    const data = await res.json() as { results: { index: number; relevance_score: number }[] };
...
**Swapping rerankers (one line in `buildQueryEngine`):**

```typescript
// v3 default — no latency, no external calls:
const reranker = new NoOpReranker();

// BGE-Reranker local server:
const reranker = new CrossEncoderReranker({ provider: 'bge', endpoint: 'http://localhost:8080' });

// Jina Reranker v2 (JINA_API_KEY env var):
const reranker = new CrossEncoderReranker({ provider: 'jina' });

// Cohere Rerank v3.5 (COHERE_API_KEY env var):
const reranker = new CrossEncoderReranker({ provider: 'cohere' });
```
...
#### `src/search/hybrid/HybridRetriever.ts` *(updated — reranker injected)*
...
export class HybridRetriever {
  ...
  async retrieve(query: string): Promise<SearchHit[]> {
    ...
    // 2. Fuse with RRF.
    const fused = rrfFusion([bm25Hits, vectorHits], this.config.rrfK, this.config.hybridTopK);

    // 3. Rerank (NoOpReranker by default in v3).
    return this.reranker.rerank(query, fused, this.config.rerankTopK);
  }
}
```
...
- [ ] `NoOpReranker.rerank` returns at most `topK` hits, all with `source: 'reranked'`
- [ ] `NoOpReranker.rerank` preserves the input ordering
- [ ] `CrossEncoderReranker` constructor throws if the required API key env var is missing
- [ ] `CrossEncoderReranker.rerank` with a mocked fetch returns hits sorted by descending score, all with `source: 'reranked'`
...
export class QueryEngine {
  ...
  async query(queryText: string): Promise<QueryResult> {
    // 1. Retrieve hybrid hits (BM25 + Vector → RRF → Reranker)
    const hits = await this.retriever.retrieve(queryText);

    // 2. Collect seed node IDs from reranked hits (capped)
...
#### `buildQueryEngine` factory

Add to `src/cli/index.ts` or extract to `src/query/factory.ts`:
...
async function buildQueryEngine(config: Config): Promise<QueryEngine> {
  ...
  const reranker  = new NoOpReranker();                                // ← swap here
  const retriever = new HybridRetriever(bm25, vector, reranker, config);
  ...
  const llm       = new PlaceholderLlmProvider();                      // ← swap here
...
- [ ] All hits in `QueryResult.hits` have `source: 'reranked'`
...
describe('Full pipeline E2E', () => {
  ...
  it('all returned hits are stamped reranked', async () => {
    const result = await engine.query('handlePayment');
    expect(result.hits.every(h => h.source === 'reranked')).toBe(true);
  });
...
| 7 | Hybrid retrieval + reranking + graph expansion | RRF-fused hits → reranked hits → bounded `GraphContext` |
